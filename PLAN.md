...
- Writes a white-label-ready narrative block in the "3–4 key figures + next steps" format (the exact format the case study cites as "the format clients actually read").
...
AgencyAnalytics, Swydo, and Whatagraph already ship AI narrative summaries — but gated behind a paid account and full data integration. Nothing free does instant, zero-setup, typed-in-numbers exception flagging + narrative on one page. That gap is the offer.
...
- CTA → same Calendly link, `utm_source=leak-audit`.
...
## Status
Not started. Plan only — awaiting go-ahead to build.
